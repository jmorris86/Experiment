...
class Vendor:
	...
	def print_status(self, vendor_number, city, experiment_round):
		if experiment_round == 1:
			statement = f"The vendor is a new vendor, and as such does not have a review history.\n"
		else:
			statement = f"The average rating of the vendor is {self.review_average} stars.\n"
		if self.relationship_status == "single":
			return f"Vendor {vendor_number} is {self.name} and they live in {city}.\n{statement}"
		elif self.relationship_status == "married":
			return f"Vendor {vendor_number} is {self.name} and they live with their spouse {self.partner_name} in " \
				   f"{city}.\n{statement}"
		else:
			return f"Vendor {vendor_number} is {self.name} and they live with their partner {self.partner_name} in " \
				   f"{city}.\n{statement}"
