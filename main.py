...
instruction_text_1 = f"Thank you for agreeing to take part in this online experiment. \n\nThe experiment takes place" \
					 ...
					 f"purchaser.\n\nThere are {rounds} rounds. In each round a purchaser buys" \
					 f" a product from one of the vendors. \nVendors display the average star rating of their previous " \
					 f"customers. Vendors not chosen \nby the participant are chosen at random by other purchasers," \
					 f" their payoff is then added \nto the vendor history in the following round. " \
					 ...
					 f"leave it offer\nto the vendor. If a vendor rejects an offer, each participant " \
					 f"will leave with nothing. If the vendor\naccepts, the vendor will receive the amount offered, and" \
					 f" purchasers will be paid the total payoff\nless the amount given to the vendor."

instruction_text_2 = f"The market consists of different purchaser and vendor types. The market share of" \
		...
		if entry_widget1.get() == "Name":
			...
			canvas.itemconfig(card_info,
							  text=f"\n\n{name} you have been randomly assigned to the role of Purchaser\nand "
								   f"are Type {Type}.\n\nPlease remember, Type {Type} receives on average "
								   f"{word_1} from {preferences[Type][0]} \nvendors, "
								   f"{word_2} from {preferences[Type][1]} vendors, and {word_3}"
								   f" from\n{preferences[Type][2]} vendors.", font=("Ariel", 14, "italic"))
			counter += 1
	elif counter == 3:
		text = f"It is important to attempt to guess the vendor Type based on each vendor's sale histories.\n\nAs a type " \
			   f"{Type} purchaser, buying exclusively from {preferences[Type][0]} vendors will earn you " \
			   f"£{round(1.5*utility_multiplier*rounds)} more than buying from {preferences[Type][2]} vendors.\n\n" \
			   f"If you would like to proceed to the experiment, click yes. To revisit the instructions, click no."
		response = messagebox.askquestion("Information", text)
		if response == "yes":
