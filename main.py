...
instruction_text_1 = f"Thank you for agreeing to take part in this online experiment. \n\nThe experiment takes place" \
					 f" in an online marketplace where participants are randomly assigned\nto a role of vendor or " \
					 f"purchaser.\n\nPurchasers are randomly assigned to one of two equal sized groups, A or B, and " \
					 f"vendors\nwill be randomly assigned to one of three groups, High Quality, Low Quality, or " \
					 f"Specialist\nProduct - where 40% of the population are High Quality, 40% Low Quality, and 20%\n" \
					 f"Specialist Producers. You will be informed of your own group, but not that of others." \
					 f"\n\nThe experiment consists of {rounds} rounds. In each round a purchaser must buy" \
					 f" a product from\none of the vendors. Vendors display the average star rating of their previous" \
					 f"customers.\nVendors not chosen by the user are chosen at random by a purchaser of unknown\ntype," \
					 f" their satisfaction is then added to the vendor history in the following round. The\npurchaser" \
					 f" can choose to purchase from the same vendor in subsequent rounds, or from a\ndifferent vendor." \
					 f"\n\nPurchasers receive payoff from buying a product of between 1 and 5 stars. Each\npurchase " \
					 f"results in a payoff of £{utility_multiplier}0 per star i.e. 1 star rating results in " \
					 f"£{utility_multiplier}0, 2 star rating\nresults in £{2 * utility_multiplier}0 etc."

instruction_text_2 = f"\n\nDependent on purchaser and vendor types, purchases have different probabilities of " \
					 f"resulting in\nhigher ratings and thus higher payouts. For both type A and type B purchasers, " \
					 f"the average\npurchase from a High Quality vendor results in 3.5 stars, whereas the average for" \
					 f" Low Quality\nvendors is 2.5 stars. Type A purchasers enjoy Specialty vendors and purchases " \
					 f"result in an\naverage of 4 stars, whereas Type B purchases of Specialty products result in an" \
					 f" average of 2 stars.\nPlease note, if a product results in a specific rating in a round, it may " \
					 f"not result in the same rating in\ndifferent rounds. This is to mirror real transactions, whereby, " \
					 f"transactions with the same vendor may\ndiffer between periods i.e. damage to the product etc. " \
					 f"which would result in a lower rating in that period.\n\nAt the end of round {rounds}, purchasers " \
					 f"will see a summary of total surplus generated from transactions\nwith each vendor. Purchasers " \
					 f"will then be asked to propose a one time take it or leave it offer to the\nvendor. Vendors can " \
					 f"reject offers,whereby each participant will leave with nothing from the " \
					 f"transaction.\nIf the vendor accepts the offer, purchasers will be remunerated an amount equal to" \
					 f" the total\npayoff less the amount given to the vendor."
...
def next_page():
	...
	if counter == 0:
		...
		if entry_widget1.get() == "Name":
			...
			canvas.itemconfig(card_info,
							  text=f"\n\n{name} you have been randomly assigned to the role of Purchaser\nand "
								   f"are Type {Type}. Please remember, Type {Type} prefers {preferences[Type][0]}"
								   f" vendors,\nthen {preferences[Type][1]} vendors, and least prefers "
		...
		if choice_widget2.get() == "Please Choose a Vendor":
			...
			if experiment_round < rounds + 1:
				...
				messagebox.showinfo("Purchase Payoff", f"Your purchase was rated as {utility_gen} stars "
													   ...
													   f"£{round(utility * utility_multiplier, 2)}0.\n\n"
		...
		statement = f"You will now be asked to make a one time take it or leave it offer to each vendor.\nYou can offer" \
					f" any amount up to the total payoff generated from purchases from\nthat vendor.\n\nPlease note, " \
					f"vendors are real people and can reject offers. In the event the offer is\nrejected, you will leave" \
					f" with nothing. If the vendor accepts, you will be paid an\namount equal to the total payoff from" \
					f" the vendor less the amount paid to them. \n\nYou need to consider both the likelihood " \
					f"of the offer being accepted and how much\nyou are willing to give away to each vendor."
		canvas.itemconfig(card_info, text=statement)
	elif counter == 6:
		canvas.itemconfig(card_info, text="")
		...
		text = f"{name}, you made the following offers to each vendor:\n\n Vendor 1 - £{offer_widget1.get()}0 out " \
			   f"of a total payoff of £{round(utility_a * utility_multiplier, 2)}0\n\nVendor 2 - " \
			   f"£{offer_widget2.get()}0 out of a total payoff of £{round(utility_b * utility_multiplier, 2)}0\n\n " \
			   f"Vendor 3 - £{offer_widget3.get()}0 out of a total payoff of " \
			   f"£{round(utility_c * utility_multiplier, 2)}0\n\nAre you happy to confirm these offers?"
		...
		if response == "yes":
			...
			if utility_c > 0:
				if np.random.choice(offer_dist, p=accepted_offers) < (
					...
					statement_c = f"Your offer to Vendor 3 has been accepted, after Vendor payment, your\nearnings from" \
								  f" Vendor 3 are £{round(earnings_c, 2)}0."
				else:
					earnings_c = 0
			...
			statement = f"{statement_a}\n\n {statement_b}\n\n{statement_c}\n\nIn addition to the flat fee of " \
						f"£{flat_fee}, this gives you total income of £" \
						f"{flat_fee + round(earnings_a + earnings_b + earnings_c, 2)}0. You will\nreceive the money into" \
...
card_title = canvas.create_text(500, 30, text="Instructions (1 of 2)", font=("Ariel", 30, "italic"))
